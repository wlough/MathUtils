...
/**
 * @brief Solve for real spherical-harmonic coefficients that best
 * fit a 3-D point cloud using least squares.
 *
 * The least squares problem is equivalent to solving the linear system
 *     (Y^T Y + \lambda L^T L) A = Y^T X,
 * where
 *     * Y - matrix of real harmonics samples, shape (num_points, (l_max+1)^2)
 *     * L - diagonal matrix with entries l(l+1)
 *     * X - input Cartesian coordinates, shape (num_points,3)
 * The result A(i,:) gives the (x,y,z) coefficients for harmonic index *i*.
 *
 * @param XYZ0        (num_points,3) matrix of Cartesian samples.
 * @param l_max       Highest harmonic degree to include.
 * @param reg_lambda  Non-negative smoothing parameter (λ = 0 --> plain LS).
...
////////////////////////////////////////////////////////////////////////////////
// Alternative implementations of spherical harmonics using expansions in powers
// of trig functions. May be unstable for large l.
 ...
 * \f$l\f$ and \f$m\f$.
 */
Matrix<double> series_real_Ylm(int l, int m,
                               const Matrix<double> &thetaphi_coord_P);
 ...
 * \f$l\f$ and \f$m\f$.
 */
Matrix<std::complex<double>> series_Ylm(int l, int m,
                                        const Matrix<double> &thetaphi_coord_P);
 ...
 */
Matrix<double>
compute_all_series_real_Ylm(int l_max, const Matrix<double> &thetaphi_coord_P);
 ...
 */
Matrix<std::complex<double>>
compute_all_series_Ylm(int l_max, const Matrix<double> &thetaphi_coord_P);
 ...
 * \f$l\f$ and \f$m\f$.
 */
Matrix<double> old_compute_all_real_Ylm(int l_max,
                                        const Matrix<double> &thetaphi_coord_P);

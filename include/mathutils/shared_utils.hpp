 ...
 */
inline Matrix<double> rthetaphi_from_xyz(const Matrix<double> &xyz) {
  if (xyz.cols() != 3) {
    throw std::invalid_argument("xyz must have 3 columns");
  }
  size_t num_points = xyz.rows();
  Matrix<double> rthetaphi(num_points, 3);
  rthetaphi.fill(0.0);
 ...
 */
inline Matrix<double> xyz_from_rthetaphi(const Matrix<double> &rthetaphi) {
  if (rthetaphi.cols() != 3) {
    throw std::invalid_argument("rthetaphi must have 3 columns");
  }
  int num_points = rthetaphi.rows();
  Matrix<double> xyz(num_points, 3);
  for (int i = 0; i < num_points; ++i) {
 ...
 */
inline Matrix<double> thetaphi_from_xyz(const Matrix<double> &xyz) {
  if (xyz.cols() != 3) {
    throw std::invalid_argument("xyz must have 3 columns");
  }
  int num_points = xyz.rows();
  Matrix<double> thetaphi(num_points, 2);
  for (int i = 0; i < num_points; ++i) {
    double x = xyz(i, 0);
    double y = xyz(i, 1);
    double z = xyz(i, 2);

...
//       |
//       ((v & 0x00ff000000000000LL) >> 40) | ((v & 0xff00000000000000LL) >>
//       56));
// }
// template <>
...
//                              size_t &srcOffset, const size_t &stride)
//                              noexcept;
// };
...
// // non-ideal. The property lookup table flattens the table down into a 2D
// array
// // optimized for parsing. The first index is the element, and the second
// index
// // is the property.
...
//       auto cursorIt = userData.find(hash_fnv1a(element.name +
//       property.name)); if (cursorIt != userData.end())
...
//         // If we didn't receive any list hints, it means we did two passes
//         over
...
//               os,
//               (helper->data->buffer.get_const() +
//               helper->cursor->byteOffset), helper->cursor->byteOffset,
...
// std::shared_ptr<PlyData>
// PlyFile::PlyFileImpl::request_properties_from_element(
//     const std::string &elementKey, const std::vector<std::string>
//     propertyKeys, const uint32_t list_size_hint) {
//   if (elements.empty())
//     throw std::runtime_error("header had no elements defined. malformed
//     file?");
//   if (elementKey.empty())
...
//     // Each key in `propertyKey` gets an entry into the userData map (keyed
//     by a
//     // hash of element name and property name), but groups of properties
//     // (requested from the public api through this function) all share the
//     same
//     // `ParsingHelper`. When it comes time to .read(), we check the number of
//     // unique PlyData shared pointers and allocate a single buffer that will
//     be
...
//     const std::string &elementKey, const std::vector<std::string>
//     propertyKeys, const Type type, const size_t count, const uint8_t *data,
...
//   read
//   // the file.
//   auto read_list_binary = [this](const Type &t, void *dst, size_t
//   &destOffset,
//                                  const size_t &stride,
...
//                        _is); // the list size
//       return read_property_binary(f.prop_stride * listSize, dest +
//       destOffset,
//                                   destOffset, destSize,
...
//       }
//       read_list_binary(p.listType, &listSize, dummyCount, f.list_stride,
//                        _is); // the list size (does not count for memory
//                        alloc)
...
//           read_property_ascii(p.propertyType, f.prop_stride, dest +
//           destOffset,
//                               destOffset, destSize, _is);
//         }
//       }
...
//             sizeof(listSize), _is); // the list size (does not count for
//             memory alloc)
//         for (size_t i = 0; i < listSize; ++i)
...
//             // structure contains enough info to write it back out again
//             (e.g.
//             // transcoding).
...
// } void PlyFile::read(std::istream &is) { return impl->read(is); } void
// PlyFile::write(std::ostream &os, bool isBinary) {
//   return impl->write(os, isBinary);
// }
// std::vector<PlyElement> PlyFile::get_elements() const { return
// impl->elements; } std::vector<std::string> &PlyFile::get_comments() { return
// impl->comments; } std::vector<std::string> PlyFile::get_info() const { return
// impl->objInfo; } bool PlyFile::is_binary_file() const { return
// impl->isBinary; } std::shared_ptr<PlyData>
// PlyFile::request_properties_from_element(
//     const std::string &elementKey, const std::vector<std::string>
//     propertyKeys, const uint32_t list_size_hint) {
...
//     propertyKeys, const Type type, const size_t count, const uint8_t *data,
//     const Type listType, const size_t listCount) {
//   return impl->add_properties_to_element(elementKey, propertyKeys, type,
//   count,

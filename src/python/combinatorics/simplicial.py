...
class _SignedSimplex:
    ...
    def __eq__(self, other):
        if isinstance(other, _SignedSimplex):
            return (
    ...
    def __neg__(self):
        return _SignedSimplex(self.sorted, -self.multiplicity)

    def __add__(self, other):
        if self.points != other.points:
            raise ValueError("Incompatible point sets")
        else:
            return _SignedSimplex(self.sorted, self.multiplicity + other.multiplicity)

    def __rmul__(self, value):
        return _SignedSimplex(self.sorted, value * self.multiplicity)
    ...
    def boundary(self):
        ...
        m = self.multiplicity
        return [
            _SignedSimplex.from_points_and_multiplicity(
                (v for v in s if v != vi), (-1) ** i * m
            )
            for i, vi in enumerate(s)
        ]

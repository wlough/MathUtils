...
# from ..pybrane_utils.misc_utils import log_log_fit, round_to
from .misc_utils import log_log_fit, round_to
import os
import subprocess
...
def to_scinotation_tex(X, decimals=3, mode="inline"):
    ...
    xlabels = [left + f"{c}" + r" \times " + r"10^{" + f"{p}" + r"}" + right for c, p in zip(coeff, pow)]
    return xlabels
...
def eq_tex_str(lhs, rhs, mode="inline"):
    if mode == "inline":
        ...
        tex_str = r"\begin{equation}" + latex(lhs) + " = " + latex(rhs) + r"\end{equation}"
    elif mode == "equation*":
        tex_str = r"\begin{equation*}" + latex(lhs) + " = " + latex(rhs) + r"\end{equation*}"

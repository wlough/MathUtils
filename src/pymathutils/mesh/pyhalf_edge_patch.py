...
class HalfEdgePatch:
    ...
    def __init__(
        ...
        self.H = H
        self.F = F
        if h_negative_B is None and find_h_negative_B:
            self.h_negative_B = self.find_h_negative_B()
        else:
    ...
    def b_left_h(self, h):
        ...
        # else:
        for b, h_start in enumerate(self.h_negative_B):
    ...
    def h_right_f(self, f):
        ...
        return self.supermesh.h_right_f(f)
    ...
    def find_h_negative_B(self, F_need2check=None):
        """works"""
        h_negative_B = []
        bdry_count = 0
        ...
        return np.array(h_negative_B, dtype=int)

    def find_h_negative_B_from_VHF(self, V_check=None, H_check=None, F_check=None):
        """
        Check supermesh.closure(V_check, H_check, F_check) for h_negative_B
        """
        if V_check is None:
            V_check = set()
        if H_check is None:
        ...
        while V_check:
            ...
            for hv in self.supermesh.generate_H_out_v_clockwise(v):
                for h in [hv, self.supermesh.h_next_h(hv)]:
    ...
    def expand_by_one_ring(self):
        ...
        self.H.update(H_new)
        self.F.update(F_new)
        self.h_negative_B = self.find_h_negative_B(F_need2check=F)
    ...
    def move_towards_cylinder(self, p0, r_max, ez):
        ...
        while h_negative_B:
            ...
            for h in self.generate_H_next_h(h):
                h_negative_B.discard(h)
        self.h_negative_B = np.array(h_negative_B, dtype=int)
    ...
    def he_samples(self):
        ...
        h_out_V = np.array([H.index(self.h_out_v(v)) for v in V], dtype=int)
        v_origin_H = np.array([V.index(self.v_origin_h(h)) for h in H], dtype=int)
        h_next_H = np.array([H.index(self.h_next_h(h)) for h in H], dtype=int)
        h_twin_H = np.array([H.index(self.h_twin_h(h)) for h in H], dtype=int)
        f_left_H = np.array(
            [
        ...
        h_right_F = np.array([H.index(self.h_right_f(f)) for f in F], dtype=int)
        h_negative_B = np.array([H.index(h) for h in self.h_negative_B], dtype=int)
        return (
            xyz_coord_V,
            h_out_V,
            v_origin_H,
    ...
    def data_lists(self):
        ...
        h_right_F = [H.index(self.h_right_f(f)) for f in F]
    ...
    def _generate_H_negative_bdry(self):
        for bdry, h in self.h_negative_B.items():
            for h in self.generate_H_next_h(h):
                yield h
    ...
    def _expand_towards_cylinder(self, x0, r_max, ez):
        ...
        self.H.update(H_new)
        self.F.update(F_new)
        self.h_negative_B = self.find_h_negative_B_from_VHF(

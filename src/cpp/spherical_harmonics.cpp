...
Matrix<std::complex<double>>
series_Ylm(int l, int m, const Matrix<double> &thetaphi_coord_P) {
  ...
  int abs_m = std::abs(m);
  int num_points = thetaphi_coord_P.rows();
  Matrix<std::complex<double>> Y(num_points);
  for (int p = 0; p < num_points; ++p) {
    double theta = thetaphi_coord_P(p, 0);
    double phi = thetaphi_coord_P(p, 1);
    if (theta < 0 || theta > M_PI) {
...
Matrix<double>
old_compute_all_real_Ylm(int l_max, const Matrix<double> &thetaphi_coord_P) {
  // check l_max
  if (l_max < 0) {
    throw std::out_of_range("l_max must be non-negative");
  }
